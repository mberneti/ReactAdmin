...
(function($){
	...
	var Redactor = function(element, options)
	{
		...
		this.opts = $.extend({
		    ...
		    /* Hakan : INSERT CODE */
			modal_code :
            '<label>' + RLANG.code + ':</label>' +
            '<textarea cols="0" id="redactor_insert_code_area" name="redactor_insert_code_area" rows="0" style="height: 211px; width: 538px;"></textarea>' +
            '<label>' + RLANG.prog_lang + '</label>' +
	...
	Redactor.prototype = {
		...
		build: function(mobile)
		{
			...
			if (mobile !== true)
			{
			    //var str = '<div class="redactor_box"></div>';
			    //if (dir === 'rtl')
			    //    str = '<div class="redactor_box ' + dir + '"></div>';
				...
				if (this.opts.imageGetJson !== false)
				{
					$.getJSON(this.opts.imageGetJson, $.proxy(function(data) {
						...
						$.each(data, $.proxy(function(key, val)
						{
							...
							var img = $('<p><img src="' + val.thumb + '" class="redactorfolder redactorfolder' + folderkey + '" rel="' + val.image + '" title="' + thumbtitle + '" /></p>');
		...
		showCode: function () {
		    ...
		    var handler = $.proxy(function () {
		        ...
		        if (this.opts.codeElement) {
		            $("#redactor_insert_code_btn").val(RLANG.code_update);
		        }

		        //console.log("Handler - ", this.opts.codeElement);

		        $('#redactor_insert_code_area').val(currentCode);
		        $('#redactor_insert_code_btn').click($.proxy(this.codeCallback, this));
		    }, this);

		    var endCallback = $.proxy(function() {
		        if (this.opts.codeElement) {
		            var lang = $(this.opts.codeElement).attr("language");
		            $("#redactor_insert_code_lang").val(lang);
		            //console.log("endCallback - lang - ", lang);
		        }
		        
		        //console.log("endCallback - ", this.opts.codeElement);
		        $("#redactor_insert_code_area").focus();
		    }, this);
		    
		    this.modalInit(RLANG.prog_code, 'code', 600, handler, endCallback);
		},
		codeCallback: function () {
		    var lang = $("#redactor_insert_code_lang").val();
		    var code = $("#redactor_insert_code_area").val();
		    code = $("<span/>").html(code).text();
		    
		    // for entering content after code
		    // without it cursor can't jump from code
		    var newLine = "<br/>";
		    ...
		    if (this.opts.codeElement) {
		        ...
		        preBlock = $(this.opts.codeElement);
		    } else {
		        preBlock = $("<pre/>");
		    }
		    ...
		    preBlock
		        // id : for find element in dom ( find for moving cursor to end of codeblock )
		        //.attr("id", randId)
		        .attr("language", lang)
		        .attr("name", "code")
		        .html(code);
		    ...
		    //    range.selectNodeContents(preBlock);//Select the entire contents of the element with the range
		    ...
		    //    range.moveToElementText(preBlock);//Select the entire contents of the element with the range
		    //    range.collapse(false);//collapse the range to the end point. false means collapse to end rather than the start
		...
		uploadFrame : function()
		{
			...
			var $div = $('<div>');
			var $iframe = $("<iframe>", {id: this.id, "name": this.id,style:"display:none"});
			...
			window.addEventListener('message', $.proxy(receiveMessage,this), false);
			...
			function receiveMessage(evt)
			{
				...
				if (this.uploadOptions.success)
				{
					if (typeof evt.data !== 'undefined')
					{
						...
						var rawString =evt.data;// d.body.innerHTML;
		...
		uploadLoaded : function()
		{
			...
			script.text  = "window.parent.postMessage(document.documentElement.innerHTML, '*')";

			var myIframe = document.getElementById(this.id);
			myIframe.appendChild(script);
